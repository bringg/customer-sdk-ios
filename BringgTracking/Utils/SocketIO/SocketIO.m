...
- (id) initWithDelegate:(id<SocketIODelegate>)delegate
{
    ...
    if (self) {
        ...
        _preferredTransport = SocketIOTransportNone;
...
- (void) onData:(NSString *)data
{
    ...
    if ([test count] > 0 &&
        [[[test objectAtIndex:0] objectAtIndex:1] intValue] >= 0 &&
        [[[test objectAtIndex:0] objectAtIndex:1] intValue] <= 8) {
...
{ 	
    ...
    if ([_sid length] < 1 || [data count] < 4) {
        ...
        switch (_preferredTransport) {
            case SocketIOTransportXHRPolling:
            ...
            case SocketIOTransportWebSocket:
            default:
